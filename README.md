...
- **更多向量数据库提供商**：如果你有兴趣与ChatGPT检索插件集成另一个向量数据库提供商，请随时提交实现。
 ...
 - **混合搜索/ TF-IDF选项**：增强[datastore的upsert函数](/datastore/datastore.md#upsert)，以允许混合搜索使用语义向量和TF-IDF分数，可以提供更准确的检索结果。使用混合搜索或TF-IDF索引选项可能会提高基于关键字的查询插件的性能。
- **多语言支持**：添加对多种语言的支持将允许插件在更广泛的上下文中使用，并提高工具的包容性。我们期待着看到ChatGPT检索插件将如何继续发展并满足其用户的需求！
